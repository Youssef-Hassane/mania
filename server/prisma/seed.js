...
for (const gov of governateArray) {
  for (const city of gov.city) {
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      if (
        hotelUrlIndexSet.has(urlIndex) ||
        hotelPrePostSet.has(
          JSON.stringify([randomIndexPreFix, randomIndexPostFix])
        )
      ) {
        i -= 1;
      } else {
    ...
    cityHotelObject[`${city.toLowerCase()}`] = hotelArray;
  }
}
console.log(cityHotelObject);
...
async function main() {
    ...
    for (const object of governateArray) {
      const governate = await prisma.governate.create({
        data: {
          ...
          cities: {
            create: object.city.map((cityName) => ({
  ...
  if (userObject.length === 0) {
    for (let i = 0; i < 300; i += 1) {
      const newUser = {};
      newUser.firstName =
        firstNameArray[getRandomInt(0, firstNameArrayLen - 1)];
      ...
      newUser.email = `${newUser.firstName}_${newUser.lastName}_${getRandomInt(
        0,
        100
      )}@${emailDomainArray[getRandomInt(0, emailDomainArray.length - 1)]}.com`;
  ...
  if (
    ...
    for (let i = 0; i < userObject.length; i += 1) {
      ...
      for (
        ...
        const randomReservationFromDate = currDate.subtract(
          getRandomInt(15, 50),
          "day"
        );
        const randomReservationToDate = currDate.subtract(
          getRandomInt(-2, 14),
          "day"
        );

        if (
          randomReservationToDate.diff(randomReservationFromDate, "day") > 30 ||
  ...
  const primaryKeyArray = passedReservations.map((some) => [
    some.userID,
    some.hotelID,
  ]);
  ...
  if (reviewObject.length === 0 && reservationObject !== 0) {
